    ...
    size_t buflen)
{
	...
	for (i = 0; i < ARRAY_SIZE(dis_riscv_2byte); i++) {
		...
		case DIS_RISCV_CL_32:
			if ((dhp->dh_flags & DIS_SIZE_32) == 0)
				continue;
			break;
		case DIS_RISCV_CL_64:
			if ((dhp->dh_flags & DIS_SIZE_64) == 0)
				continue;
			break;
		case DIS_RISCV_CL_32_64:
			if ((dhp->dh_flags &
			    (DIS_SIZE_32 | DIS_SIZE_64)) == 0) {
				continue;
			}
			break;
...
{
	if ((flags & DIS_ARCH_MASK) == DIS_RISCV &&
	    DIS_SIZE_CHECK(flags, DIS_SIZE_32 | DIS_SIZE_64))
		return (1);
