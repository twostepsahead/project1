...
{
	assert(separator_1st != SIP_NUL || separator_2nd != SIP_NUL);
	while (sip_header->sip_hdr_current < sip_header->sip_hdr_end) {
    ...
    boolean_t parse_uri)
{
	...
	while (hdr->sip_hdr_current < hdr->sip_hdr_end) {
		...
		r = sip_find_separator(hdr, SIP_COMMA, SIP_SEMI, SIP_NUL,
		    B_FALSE);
		if (r != 0) {
			value->sip_value_end = hdr->sip_hdr_current;
			goto end;
    ...
    boolean_t parse_uri)
{
	...
	for (;;) {
		...
		if (sip_find_separator(hdr, SIP_EQUAL, SIP_COMMA, SIP_NUL,
		    B_FALSE) != 0) {
