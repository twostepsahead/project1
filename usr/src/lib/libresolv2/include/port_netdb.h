 ...
 * res_* because they conflict with libnsl or libc
 */

#define	endhostent res_endhostent /* libnsl */
void endhostent __P((void));
#define	endnetent res_endnetent  /* libc */
void endnetent __P((void));
#define	freeaddrinfo res_freeaddrinfo /* libc */
void freeaddrinfo __P((struct addrinfo *));
#define	freehostent res_freehostent  /* libc and libnsl */
void freehostent __P((struct hostent *));
#define	getaddrinfo res_getaddrinfo  /* libc */
...
#define	getipnodebyaddr res_getipnodebyaddr  /* libnsl and libc */
struct hostent *getipnodebyaddr __P((const void *, size_t, int, int *));
#define	getipnodebyname res_getipnodebyname  /* libnsl and libc */
struct hostent *getipnodebyname __P((const char *, int, int, int *));

#define	getnetbyaddr res_getnetbyaddr /* libc */
struct netent *getnetbyaddr __P((unsigned long, int));
#define	getnetbyname res_getnetbyname /* libc */
struct netent *getnetbyname __P((const char *));
#define	getnetent res_getnetent /* libc */
struct netent *getnetent __P((void));
#define	sethostent res_sethostent /* libnsl */
void sethostent __P((int));
#define	setnetent res_setnetent /* libc */
void setnetent __P((int));

/*
 * these are other irs functions now included in libresolv.so.2. We rename the
 * ones that overlap with libc or libnsl
 */

/* endprotoent is in libc.so.1 */
#define	endprotoent res_endprotoent
void		endprotoent __P((void));

/* endservent is in libc.so.1 */
#define	endservent res_endservent
void		endservent __P((void));

/* note: the next two symbols are variables, not functions */

/* gai_errlist is in libc.so.1 */
#define	gai_errlist res_gai_errlist

/* gai_nerr is in libc.so.1 */
#define	gai_nerr res_gai_nerr

/* gai_strerror is in libc.so.1 */
