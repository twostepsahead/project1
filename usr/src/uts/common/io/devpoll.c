...
repoll:
			...
			if ((pdp->pd_events & POLLET) && error == 0 &&
				...
				/*
				 * The same POLLET-only fop_poll is used in an
				 * attempt to coax a pollhead from older
				 * driver logic.
				 */
				error = fop_poll(fp->f_vnode, POLLET,
				    0, &levent, &php, NULL);
			}
			...
			if (revent != 0) {
				if (pfdp != NULL) {
					 ...
					 * fop_poll handlers to emit, it must
...
dpwrite(dev_t dev, struct uio *uiop, cred_t *credp)
{
	...
	for (pfdp = pollfdp; (uintptr_t)pfdp < limit;
		...
		if (pfdp->events != POLLREMOVE) {
			 ...
			 * here via a second fop_poll, with only POLLET set in
			...
			if ((pdp->pd_events & POLLET) && error == 0 &&
				...
				error = fop_poll(fp->f_vnode, POLLET, 0,
				    &levent, &php, NULL);
			}
