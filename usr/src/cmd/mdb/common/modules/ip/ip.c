...
th_trace(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (mdb_pwalk("th_hash", th_hash_summary, &thw,
	    (uintptr_t)NULL) == -1) {
...
ipcl_hash_get_next_connf_tbl(ipcl_hash_walk_data_t *iw)
{
	...
	uintptr_t addr = (uintptr_t)NULL, next;
...
ncec_stack_walk_init(mdb_walk_state_t *wsp)
{
	...
	wsp->walk_addr = ncec_get_next_hash_tbl((uintptr_t)NULL,
...
ilb_rules_walk_step(mdb_walk_state_t *wsp)
{
	...
	if (status != WALK_NEXT)
		return (status);
	if ((wsp->walk_addr = (uintptr_t)rule.ir_next) == (uintptr_t)NULL)
		return (WALK_DONE);
	else
		return (WALK_NEXT);
}
...
ilb_servers_walk_init(mdb_walk_state_t *wsp)
{
	...
	if ((wsp->walk_addr = (uintptr_t)rule.ir_servers) != (uintptr_t)NULL)
		return (WALK_NEXT);
	else
		return (WALK_DONE);
}
...
ilb_servers_walk_step(mdb_walk_state_t *wsp)
{
	...
	if (status != WALK_NEXT)
		return (status);
	if ((wsp->walk_addr = (uintptr_t)server.iser_next) == (uintptr_t)NULL)
		return (WALK_DONE);
	else
		return (WALK_NEXT);
}
