...
static uintptr_t cur_iport_for_irp_loop = (uintptr_t)NULL;
...
next_rport(struct fct_i_local_port *iport)
{
	...
	if (ws.walk_addr == (uintptr_t)NULL || cur_iport_for_irp_loop !=
	    (uintptr_t)iport) {
		*((fct_i_remote_port_t **)ws.walk_cbdata) = NULL;
		...
		if (ws.walk_addr == (uintptr_t)NULL) {
			fct_irp_walk_f(&ws);
			return (NULL);
		}
	}
...
find_irp_by_wwn(struct stmf_i_local_port *siport, uint8_t wwn[8])
{
	...
	cur_iport_for_irp_loop = (uintptr_t)NULL;
	return (ret);
}
