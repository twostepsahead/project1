...
fanout_stack_walk_init(mdb_walk_state_t *wsp)
{
	...
	lw->sctp = (uintptr_t)NULL;
	lw->fanout = (sctp_tf_t *)kaddr;
	lw->getnext = fi->getnext;

	if ((wsp->walk_addr = find_next_hash_item(lw)) == (uintptr_t)NULL) {
		return (WALK_DONE);
	}
	wsp->walk_data = lw;
	return (WALK_NEXT);
...
fanout_stack_walk_step(mdb_walk_state_t *wsp)
{
	...
	if ((wsp->walk_addr = find_next_hash_item(fw)) == (uintptr_t)NULL)
		return (WALK_DONE);
...
sctp_walk_faddr_init(mdb_walk_state_t *wsp)
{
	...
	if ((wsp->walk_addr = (uintptr_t)sctp.sctp_faddrs) != (uintptr_t)NULL)
		return (WALK_NEXT);
	else
		return (WALK_DONE);
}
...
sctp_walk_faddr_step(mdb_walk_state_t *wsp)
{
	...
	if (status != WALK_NEXT)
		return (status);
	if ((faddr_ptr = (uintptr_t)sctp_faddr.sf_next) == (uintptr_t)NULL) {
		return (WALK_DONE);
	} else {
