...
segvn_pages_walk_step(mdb_walk_state_t *wsp)
{
	...
	if (svw->svw_sparse != NULL) {
	...
	} else if (svd->amp == NULL || wsp->walk_addr == (uintptr_t)NULL) {
		/*
		...
		if (anon->an_vp != NULL || anon->an_off != 0) {
			...
			pp = (uintptr_t)NULL;	/* nothing at this offset */
		}
	}
