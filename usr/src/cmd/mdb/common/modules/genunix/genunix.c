...
pgrep_cb(uintptr_t addr, const void *ignored, void *data)
{
	...
	if (pgp->pg_flags & (PG_NEWEST | PG_OLDEST)) {
		...
		if (pgp->pg_flags & PG_NEWEST) {
			if (pgp->pg_xaddr == (uintptr_t)NULL ||
			    start > pgp->pg_xstart) {
				pgp->pg_xaddr = addr;
				pgp->pg_xstart = start;
			}
...
pgrep(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (pg.pg_xaddr != (uintptr_t)NULL &&
	    (pg.pg_flags & (PG_NEWEST | PG_OLDEST))) {
...
callout_t_cb(uintptr_t addr, const void *data, void *priv)
{
	...
	if (!(coargs->flags & COF_EMPTY) && (
	    (ct->ct_heap == NULL) || (ct->ct_cyclic == (uintptr_t)NULL))) {
		return (WALK_NEXT);
	}
...
lminfo(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	return (mdb_pwalk("lock_graph", lminfo_cb, NULL, (uintptr_t)NULL));
}
...
whereopen(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (!(flags & DCMD_ADDRSPEC) || addr == (uintptr_t)NULL)
		return (DCMD_USAGE);
...
proc_walk_step(mdb_walk_state_t *wsp)
{
	...
	if ((wsp->walk_addr = cld) != (uintptr_t)NULL) {
		if (mdb_ctf_vread(&pr, "proc_t", "mdb_walk_proc_t",
		    cld, 0) == -1) {
...
sib:
	...
	if (sib != (uintptr_t)NULL &&
	    mdb_ctf_vread(&pr, "proc_t", "mdb_walk_proc_t", sib, 0) == -1) {
		mdb_warn("proc %p has invalid p_sibling %p; skipping\n",
		    addr, sib);
		sib = (uintptr_t)NULL;
	}

	if ((wsp->walk_addr = sib) == (uintptr_t)NULL) {
		if (pw->pw_depth > 0) {
...
ptree_walk(uintptr_t addr, const void *ignored, void *data)
{
	...
	for (paddr = (uintptr_t)proc.p_parent; paddr != (uintptr_t)NULL; ident += 5) {
...
pid2proc(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if ((addr = mdb_pid2proc(pid, NULL)) == (uintptr_t)NULL) {
