...
qlclinks_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	uintptr_t		hbaptr = (uintptr_t)NULL;
	...
	while (hbaptr != (uintptr_t)NULL) {
		...
		if ((qlstate->flags & VP_ENABLED) &&
			...
			uintptr_t		vhbaptr = (uintptr_t)NULL;
