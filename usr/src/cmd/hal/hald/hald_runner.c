			       ...
			       void *user_data)
{
	...
	if (dbus_message_is_signal (message, 
		...
		if (dbus_message_get_args (message, &error,
			...
			rp = g_hash_table_lookup (running_processes, (gpointer) (intptr_t) pid);
			if (rp != NULL) {
				rp->cb (rp->device, 0, 0, NULL, rp->data1, rp->data2);
				g_hash_table_remove (running_processes, (gpointer) (intptr_t) pid);
				g_free (rp);
			}
		}
	}
  ...
  if (reply) {
	...
	if (dbus_message_get_args (reply, &err,
		...
		if (cb != NULL) {
			...
			g_hash_table_insert (running_processes, (gpointer) (intptr_t) rp->pid, rp);
		}
	} else {
