...
const struct vnodeops nfs_vnodeops = {
	.vnop_name = "nfs2",
	.vop_open = nfs_open,
	.vop_close = nfs_close,
	.vop_read = nfs_read,
	...
	.vop_fid = nfs_fid,
	.vop_rwlock = nfs_rwlock,
	.vop_rwunlock = nfs_rwunlock,
	.vop_seek = nfs_seek,
	.vop_frlock = nfs_frlock,
	.vop_space = nfs_space,
	.vop_realvp = nfs_realvp,
	.vop_getpage = nfs_getpage,
	.vop_putpage = nfs_putpage,
	.vop_map = nfs_map,
	.vop_addmap = nfs_addmap,
	.vop_delmap = nfs_delmap,
	.vop_dump = nfs_dump,
	.vop_pathconf = nfs_pathconf,
	.vop_pageio = nfs_pageio,
	.vop_setsecattr = nfs_setsecattr,
	.vop_getsecattr = nfs_getsecattr,
	.vop_shrlock = nfs_shrlock,
	...
	caller_context_t *ct)
{
	...
	if ((flag & FWRITE) && vn_has_cached_data(vp)) {
		if ((VTOMI(vp)->mi_flags & MI_NOCTO)) {
			error = nfs_putpage(vp, 0, 0, B_ASYNC,
			    cr, ct);
			if (error == EAGAIN)
				error = 0;
		} else
			error = nfs_putpage(vp, 0, 0, 0, cr, ct);
		if (!error) {
			mutex_enter(&rp->r_statelock);
			error = rp->r_error;
			rp->r_error = 0;
		...
		caller_context_t *ct)
{
	...
	if (error == 0 && (mask & VATTR_SIZE) && vap->va_size == 0)
...
nfssetattr(vnode_t *vp, struct vattr *vap, int flags, cred_t *cr)
{
	...
	if (vn_has_cached_data(vp) &&
		...
		error = nfs_putpage(vp, 0, 0, 0, cr, NULL);
		if (error && (error == ENOSPC || error == EDQUOT)) {
			mutex_enter(&rp->r_statelock);
			if (!rp->r_error)
				rp->r_error = error;
	 ...
	 */
	if ((mask & VATTR_MTIME) && !(flags & ATTR_UTIME)) {
	 ...
	 */
	if ((mask & (VATTR_UID | VATTR_GID | VATTR_MODE)) && (mi->mi_flags & MI_ACL)) {
...
nfs_access(vnode_t *vp, int mode, int flags, cred_t *cr, caller_context_t *ct)
{
	...
	va.va_mask = VATTR_MODE | VATTR_UID | VATTR_GID;
	error = nfsgetattr(vp, &va, cr);
	if (error)
		return (error);
...
#define	IFSOCK		0140000		/* socket */
	...
	if (vattr.va_type == VCHR || vattr.va_type == VBLK) {
		...
		/*
		 * To avoid triggering bugs in the servers set VATTR_SIZE
	...
	struct flk_callback *flk_cbp, cred_t *cr, caller_context_t *ct)
{
	...
	if (cmd != F_GETLK) {
		...
		if (rc != 0)
			goto done;
		error = nfs_putpage(vp, 0, 0, B_INVAL, cr, ct);
		if (error) {
