    ...
    int copy_cnt, ssize32_t *count)
{
	...
	for (i = 0; i < copy_cnt; i++) {
		...
		if (sfv->sfv_fd == SFV_FD_SELF) {
			...
			while (sfv_len > 0) {
				error = fop_write(vp, &auio, ioflag,
				    fp->f_cred, NULL);
			...
			if (readvp < vp) {
				...
				(void) fop_rwlock(readvp, V_WRITELOCK_FALSE,
				    NULL);
			...
			if (sfv_off > MAXOFFSET_T) {
				fop_rwunlock(readvp, V_WRITELOCK_FALSE, NULL);
				releasef(sfv->sfv_fd);
				return (EINVAL);
			}
			...
			while (sfv_len > 0) {
				...
				if ((ioflag & FRSYNC) == 0)
					ioflag &= ~(FSYNC|FDSYNC);
				error = fop_read(readvp, &auio, ioflag,
				    fp->f_cred, NULL);
				if (error) {
					kmem_free(ptr, size);
					fop_rwunlock(readvp, V_WRITELOCK_FALSE,
					    NULL);
					releasef(sfv->sfv_fd);
					return (error);
				}
    ...
    int copy_cnt, ssize_t *count)
{
	...
	for (i = 0; i < copy_cnt; i++) {
		...
		if (sfv->sfv_fd == SFV_FD_SELF) {
			...
			readvp = ffp->f_vnode;
			if (fop_realvp(readvp, &realvp, NULL) == 0)
				readvp = realvp;
			if (readvp->v_type != VREG) {
				releasef(sfv->sfv_fd);
				return (EINVAL);
