...
struct fem {
	...
	int (*femop_open)(femarg_t *vf, int mode, cred_t *cr,
			  caller_context_t *ct);
	int (*femop_close)(femarg_t *vf, int flag, int count, offset_t offset,
			   cred_t *cr, caller_context_t *ct);
	int (*femop_read)(femarg_t *vf, uio_t *uiop, int ioflag, cred_t *cr,
			  caller_context_t *ct);
	int (*femop_write)(femarg_t *vf, uio_t *uiop, int ioflag, cred_t *cr,
			   caller_context_t *ct);
	int (*femop_ioctl)(femarg_t *vf, int cmd, intptr_t arg, int flag,
			   cred_t *cr, int *rvalp, caller_context_t *ct);
	int (*femop_setfl)(femarg_t *vf, int oflags, int nflags, cred_t *cr,
			   caller_context_t *ct);
	int (*femop_getattr)(femarg_t *vf, vattr_t *vap, int flags, cred_t *cr,
			     caller_context_t *ct);
	int (*femop_setattr)(femarg_t *vf, vattr_t *vap, int flags, cred_t *cr,
			     caller_context_t *ct);
	int (*femop_access)(femarg_t *vf, int mode, int flags, cred_t *cr,
			    caller_context_t *ct);
	int (*femop_lookup)(femarg_t *vf, char *nm, struct vnode **vpp,
	...
	int (*femop_link)(femarg_t *vf, struct vnode *svp, char *tnm,
			  cred_t *cr, caller_context_t *ct, int flags);
	int (*femop_rename)(femarg_t *vf, char *snm, struct vnode *tdvp,
	...
	int (*femop_mkdir)(femarg_t *vf, char *dirname, vattr_t *vap,
			   ...
			   caller_context_t *ct, int flags,
			   vsecattr_t *vsecp);
	int (*femop_rmdir)(femarg_t *vf, char *nm, struct vnode *cdir,
			   cred_t *cr, caller_context_t *ct, int flags);
	...
	int (*femop_cmp)(femarg_t *vf, struct vnode *vp2,
			 caller_context_t *ct);
	...
	int (*femop_space)(femarg_t *vf, int cmd, struct flock64 *bfp,
			   ...
			   caller_context_t *ct);
	int (*femop_realvp)(femarg_t *vf, struct vnode **vpp,
			    caller_context_t *ct);
...
struct fsem {
	const char *name;
	int (*fsemop_mount)(fsemarg_t *vf, struct vnode *mvp,
			    struct mounta *uap, cred_t *cr);
	int (*fsemop_unmount)(fsemarg_t *vf, int flag, cred_t *cr);
	int (*fsemop_root)(fsemarg_t *vf, struct vnode **vpp);
	int (*fsemop_statvfs)(fsemarg_t *vf, statvfs64_t *sp);
	int (*fsemop_sync)(fsemarg_t *vf, short flag, cred_t *cr);
	int (*fsemop_vget)(fsemarg_t *vf, struct vnode **vpp, fid_t *fidp);
	int (*fsemop_mountroot)(fsemarg_t *vf, enum whymountroot reason);
	void (*fsemop_freevfs)(fsemarg_t *vf);
	int (*fsemop_vnstate)(fsemarg_t *vf, struct vnode *vp, vntrans_t nstate);
};


extern int vhead_open(vnode_t **vpp, int mode, cred_t *cr,
		      caller_context_t *ct);
extern int vhead_close(vnode_t *vp, int flag, int count, offset_t offset,
		       cred_t *cr, caller_context_t *ct);
extern int vhead_read(vnode_t *vp, uio_t *uiop, int ioflag, cred_t *cr,
		      caller_context_t *ct);
extern int vhead_write(vnode_t *vp, uio_t *uiop, int ioflag, cred_t *cr,
		       caller_context_t *ct);
extern int vhead_ioctl(vnode_t *vp, int cmd, intptr_t arg, int flag, cred_t *cr,
		       int *rvalp, caller_context_t *ct);
extern int vhead_setfl(vnode_t *vp, int oflags, int nflags, cred_t *cr,
		       caller_context_t *ct);
extern int vhead_getattr(vnode_t *vp, vattr_t *vap, int flags, cred_t *cr,
			 caller_context_t *ct);
extern int vhead_setattr(vnode_t *vp, vattr_t *vap, int flags, cred_t *cr,
			 caller_context_t *ct);
extern int vhead_access(vnode_t *vp, int mode, int flags, cred_t *cr,
			caller_context_t *ct);
extern int vhead_lookup(vnode_t *dvp, char *nm, vnode_t **vpp, pathname_t *pnp,
			int flags, vnode_t *rdir, cred_t *cr,
			caller_context_t *ct, int *direntflags,
			pathname_t *realpnp);
extern int vhead_create(vnode_t *dvp, char *name, vattr_t *vap, vcexcl_t excl,
			int mode, vnode_t **vpp, cred_t *cr, int flag,
			caller_context_t *ct, vsecattr_t *vsecp);
extern int vhead_remove(vnode_t *dvp, char *nm, cred_t *cr,
			caller_context_t *ct, int flags);
extern int vhead_link(vnode_t *tdvp, vnode_t *svp, char *tnm, cred_t *cr,
		      caller_context_t *ct, int flags);
extern int vhead_rename(vnode_t *sdvp, char *snm, vnode_t *tdvp, char *tnm,
			cred_t *cr, caller_context_t *ct, int flags);
extern int vhead_mkdir(vnode_t *dvp, char *dirname, vattr_t *vap, vnode_t **vpp,
		       cred_t *cr, caller_context_t *ct, int flags,
		       vsecattr_t *vsecp);
extern int vhead_rmdir(vnode_t *dvp, char *nm, vnode_t *cdir, cred_t *cr,
		       caller_context_t *ct, int flags);
extern int vhead_readdir(vnode_t *vp, uio_t *uiop, cred_t *cr, int *eofp,
			 caller_context_t *ct, int flags);
extern int vhead_symlink(vnode_t *dvp, char *linkname, vattr_t *vap,
			 char *target, cred_t *cr, caller_context_t *ct,
			 int flags);
extern int vhead_readlink(vnode_t *vp, uio_t *uiop, cred_t *cr,
			  caller_context_t *ct);
extern int vhead_fsync(vnode_t *vp, int syncflag, cred_t *cr,
		       caller_context_t *ct);
extern void vhead_inactive(vnode_t *vp, cred_t *cr, caller_context_t *ct);
extern int vhead_fid(vnode_t *vp, fid_t *fidp, caller_context_t *ct);
extern int vhead_rwlock(vnode_t *vp, int write_lock, caller_context_t *ct);
extern void vhead_rwunlock(vnode_t *vp, int write_lock, caller_context_t *ct);
extern int vhead_seek(vnode_t *vp, offset_t ooff, offset_t *noffp,
		      caller_context_t *ct);
extern int vhead_cmp(vnode_t *vp1, vnode_t *vp2, caller_context_t *ct);
extern int vhead_frlock(vnode_t *vp, int cmd, struct flock64 *bfp, int flag,
			offset_t offset, struct flk_callback *flk_cbp,
			cred_t *cr, caller_context_t *ct);
extern int vhead_space(vnode_t *vp, int cmd, struct flock64 *bfp, int flag,
		       offset_t offset, cred_t *cr, caller_context_t *ct);
extern int vhead_realvp(vnode_t *vp, vnode_t **vpp, caller_context_t *ct);
extern int vhead_getpage(vnode_t *vp, offset_t off, size_t len, uint_t *protp,
			 struct page **plarr, size_t plsz, struct seg *seg,
			 caddr_t addr, enum seg_rw rw, cred_t *cr,
			 caller_context_t *ct);
extern int vhead_putpage(vnode_t *vp, offset_t off, size_t len, int flags,
			 cred_t *cr, caller_context_t *ct);
extern int vhead_map(vnode_t *vp, offset_t off, struct as *as, caddr_t *addrp,
		     size_t len, uchar_t prot, uchar_t maxprot, uint_t flags,
		     cred_t *cr, caller_context_t *ct);
extern int vhead_addmap(vnode_t *vp, offset_t off, struct as *as, caddr_t addr,
			size_t len, uchar_t prot, uchar_t maxprot, uint_t flags,
			cred_t *cr, caller_context_t *ct);
extern int vhead_delmap(vnode_t *vp, offset_t off, struct as *as, caddr_t addr,
			size_t len, uint_t prot, uint_t maxprot, uint_t flags,
			cred_t *cr, caller_context_t *ct);
extern int vhead_poll(vnode_t *vp, short events, int anyyet, short *reventsp,
		      struct pollhead **phpp, caller_context_t *ct);
extern int vhead_dump(vnode_t *vp, caddr_t addr, offset_t lbdn, offset_t dblks,
		      caller_context_t *ct);
extern int vhead_pathconf(vnode_t *vp, int cmd, ulong_t *valp, cred_t *cr,
			  caller_context_t *ct);
extern int vhead_pageio(vnode_t *vp, struct page *pp, uoff_t io_off,
			size_t io_len, int flags, cred_t *cr,
			caller_context_t *ct);
extern int vhead_dumpctl(vnode_t *vp, int action, offset_t *blkp,
			 caller_context_t *ct);
extern void vhead_dispose(vnode_t *vp, struct page *pp, int flag, int dn,
			  cred_t *cr, caller_context_t *ct);
extern int vhead_setsecattr(vnode_t *vp, vsecattr_t *vsap, int flag, cred_t *cr,
			    caller_context_t *ct);
extern int vhead_getsecattr(vnode_t *vp, vsecattr_t *vsap, int flag, cred_t *cr,
			    caller_context_t *ct);
extern int vhead_shrlock(vnode_t *vp, int cmd, struct shrlock *shr, int flag,
			 cred_t *cr, caller_context_t *ct);
extern int vhead_vnevent(vnode_t *vp, vnevent_t vnevent, vnode_t *dvp,
			 char *cname, caller_context_t *ct);
extern int vhead_reqzcbuf(vnode_t *vp, enum uio_rw ioflag, xuio_t *xuiop,
			  cred_t *cr, caller_context_t *ct);
extern int vhead_retzcbuf(vnode_t *vp, xuio_t *xuiop, cred_t *cr,
			  caller_context_t *ct);

extern int fshead_mount(vfs_t *vfsp, vnode_t *mvp, struct mounta *uap,
			cred_t *cr);
extern int fshead_unmount(vfs_t *vfsp, int flag, cred_t *cr);
extern int fshead_root(vfs_t *vfsp, vnode_t **vpp);
extern int fshead_statvfs(vfs_t *vfsp, statvfs64_t *sp);
extern int fshead_sync(vfs_t *vfsp, short flag, cred_t *cr);
extern int fshead_vget(vfs_t *vfsp, vnode_t **vpp, fid_t *fidp);
extern int fshead_mountroot(vfs_t *vfsp, enum whymountroot reason);
extern void fshead_freevfs(vfs_t *vfsp);
extern int fshead_vnstate(vfs_t *vfsp, vnode_t *vp, vntrans_t nstate);
