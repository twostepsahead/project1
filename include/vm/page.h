...
#define	VM_STAT_COND_ADD(cond, stat)	do { \
						if (cond) \
							VM_STAT_ADD(stat); \
					} while (0)
#else
...
int	page_lock(struct page *, se_t, struct vmobject *, reclaim_t);
int	page_lock_es(struct page *, se_t, struct vmobject *, reclaim_t, int);
